from utils import egobody_dataset, seed_torch
...
def main(opt):
    ...
    if joints_used == 'all':
        opt.joint_number = 23*2
    if joints_used == 'torso':
        opt.joint_number = 7*2
    if joints_used == 'arm':
        opt.joint_number = 8*2
    if joints_used == 'leg':
        opt.joint_number = 8*2
    if joints_used == 'torso+arm':
        opt.joint_number = 15*2
    if joints_used == 'torso+leg':
        opt.joint_number = 15*2
    if joints_used == 'arm+leg':
        opt.joint_number = 16*2
    ...
    train_dataset = egobody_dataset.egobody_dataset(data_dir, seq_len, opt.actions, joints_used = joints_used, train_flag = 1, sample_rate = opt.train_sample_rate)
    ...
    valid_dataset = egobody_dataset.egobody_dataset(data_dir, seq_len, opt.actions, joints_used = joints_used, train_flag = 0)
...
def eval(opt):
    ...
    print('>>> loading datasets')        
    test_dataset = egobody_dataset.egobody_dataset(data_dir, seq_len, opt.actions, joints_used = joints_used, train_flag = 0)
